...
RC_Data_t::RC_Data_t() {
    ...
    last_mode = -1.0;
    set_default_mode(std::string("manual")); // default last_gear: -1 (-0.45, -1)
    set_default_mode(std::string("noapi")); // default last_mode: -0.8 (-0.8, 0, 0.8)
...
void RC_Data_t::feed(sensor_msgs::JoyConstPtr pMsg) {
    ...
    std::cout << "gear " << gear << std::endl << std::endl;
    ...
    if (last_mode < API_MODE_THRESHOLD_VALUE && mode > API_MODE_THRESHOLD_VALUE) // switch the upper-left button from A to F
        enter_api_mode = true;
    else                                                                         // switch the upper-left button from F to A
        enter_api_mode = false;
    ...
    if (last_gear < GEAR_SHIFT_VALUE && gear > GEAR_SHIFT_VALUE) { // switch the H button down
        enter_command_mode = true;
    } else if (gear < GEAR_SHIFT_VALUE) {                          // switch the H button up
        enter_command_mode = false;
    }
...
Imu_MYNT_Data_t::Imu_MYNT_Data_t() {
    rcv_stamp = ros::Time(0);
}

void Imu_MYNT_Data_t::feed(sensor_msgs::ImuConstPtr pMsg) {
    msg = *pMsg;
    rcv_stamp = ros::Time::now();
