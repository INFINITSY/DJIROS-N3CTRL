...
int main(int argc, char* argv[]) {
    ...
    fsm.controller.config(); // config gain, default: hover gain
    ...
    ros::Subscriber joy_sub =
        nh.subscribe<sensor_msgs::Joy>("joy",
                                       ...
                                       ros::VoidConstPtr(),
                                       ros::TransportHints().tcpNoDelay()); // topic "/djiros/rc", recieve remote controller's signal
    ...
    ros::Subscriber odom_sub =
        nh.subscribe<nav_msgs::Odometry>("odom",
                                         ...
                                         ros::VoidConstPtr(),
                                         ros::TransportHints().tcpNoDelay()); // topic "/vins_estimator/imu_propagate", recieve odometry data
    ...
    ros::Subscriber imu_sub =
        nh.subscribe<sensor_msgs::Imu>("imu",
                                       ...
                                       ros::VoidConstPtr(),
                                       ros::TransportHints().tcpNoDelay()); // topic "/djiros/imu"

    ros::Subscriber imu_mynt_sub =
        nh.subscribe<sensor_msgs::Imu>("mynteye_imu",
                                       1000,
                                       boost::bind(&Imu_MYNT_Data_t::feed, &fsm.imu_mynt_data, _1),
                                       ros::VoidConstPtr(),
                                       ros::TransportHints().tcpNoDelay()); // topic "/mynteye/imu/data_raw"
