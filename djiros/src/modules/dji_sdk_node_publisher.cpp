...
#include "/home/clarence/teach-repeat-replan_ws/src/onboard_computer/controller/djiros/include/dji_sdk/dji_sdk_node.h"
#include <tf/tf.h>
...
DJISDKNode::dataBroadcastCallback()
{
  ...
  if (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::A3_HAS_RC)
  {
    ...
    rc_joy.axes.push_back(static_cast<float>(vehicle->broadcast->getRC().mode / 10000.0));
    rc_joy.axes.push_back(static_cast<float>(vehicle->broadcast->getRC().gear / 10000.0));
